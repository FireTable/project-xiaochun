...
export const HeadBubble: React.FC<HeadBubbleProps> = ({ state }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-[240px] sm:max-w-xs px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-2xl bg-slate-950/90 border border-white/20 shadow-2xl backdrop-blur-2xl flex flex-col gap-1 text-center ring-1 ring-brand-400/30 animate-in fade-in zoom-in-95 duration-200">
        {state.statusKey && (
          <div
            className={`flex items-center justify-center gap-1.5 text-[11px] font-medium tracking-wide ${
        ...
        {state.speechText && (
          <div className="text-sm font-medium text-white leading-snug break-words">
            {state.speechText}
          </div>
        )}
