...
export const HeadBubble: React.FC<HeadBubbleProps> = ({ state }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-[240px] sm:max-w-xs px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-2xl bg-slate-950/90 shadow-2xl backdrop-blur-2xl flex flex-col gap-1 text-center ring-1 ring-brand-400/30 animate-in fade-in zoom-in-95 duration-200 pointer-events-auto">
        {state.statusKey && (
          <div
        ...
        <svg
          aria-hidden="true"
          className="absolute top-[calc(100%-1px)] left-1/2 -translate-x-1/2 w-2.5 h-2 overflow-visible pointer-events-none z-10"
          viewBox="0 0 14 8"
          fill="none"
        >
          {/* 填充：与气泡 bg-slate-950/90 同色，无缝连成一体 */}
          <path
            d="M0 0 L14 0 L7 7.5 Z"
