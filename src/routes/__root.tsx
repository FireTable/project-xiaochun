...
export const Route = createRootRoute({
  head: () => ({
    meta: [
      ...
      { name: 'description', content: '100% browser-native anime companion — WebLLM (WebGPU) + EMAGE full-body motion (ONNX wasm + INT8, streaming T=64) + Edge-TTS. Private VRM chat in-tab, zero backend.' },
      { name: 'keywords', content: 'Project XiaoChun, 二次元伴侣, 虚拟伴侣, AI伴侣, 骨骼微调, anime companion, virtual companion, VRM, VRM morph, WebLLM, WebGPU, ONNX Runtime Web, wasm, INT8, Edge-TTS, EMAGE, streaming motion, three.js, browser-native, リアルタイムAI, バーチャルコンパニオン, 3Dアバター' },
      ...
      { name: 'DC.subject', content: '3D Anime Companion, WebLLM, WebGPU LLM, EMAGE ONNX wasm INT8, VRM' },
      ...
      { property: 'og:description', content: '100% 浏览器原生二次元伴侣 — WebLLM（可用 WebGPU）+ EMAGE 全身动作（ONNX wasm + INT8，流式 T=64）+ Edge-TTS。纯本地隐私，无需后端。' },
      ...
      { name: 'twitter:description', content: '100% 浏览器原生二次元伴侣 — WebLLM（WebGPU）+ EMAGE（wasm/INT8 流式）+ Edge-TTS。' },
    ...
    scripts: [
      {
        ...
        children: JSON.stringify({
          ...
          description: '100% 浏览器原生二次元伴侣 — WebLLM（可用 WebGPU）+ EMAGE 全身动作（ONNX wasm + INT8，流式 motion_chunk）+ Edge-TTS。纯本地隐私，无需云端后端。',
