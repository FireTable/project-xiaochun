...
```
桌面端也可以在应用内检查 GitHub Releases（顶栏 **⋯** → 检查更新）。Homebrew 与应用内更新两条通道都可用。
...
* **VRM 1.0 & VRM 0.x 双规格渲染中枢**:基于 `@pixiv/three-vrm` 与 MToon NPR 着色，核心引擎经深度模块化解耦，原生兼顾现代 **VRM 1.0** 与经典 **VRM 0.x** 模型体系：
  * **全局动作管线透明适配**:所有上层动作逻辑（待机、EMAGE、动作片段、转向踱步、Gaze）一律以 VRM 1.0 标准空间编写，由 `PoseBuffer` 在管线底层自动进行 X/Z 镜像与四元数转换，VRM 0.0 模型开箱即用，零肢体反向或扭曲。
...
* **增量 Delta 补丁分发 (`.vrmbase` + `.vrmaddon`)**：打破动辄数十兆的模型体积瓶颈，提取公用身体骨骼生成轻量底模 `xiaochun_base.vrmbase`（~5.9 MB，oxipng 极致压缩纹理）；9 套风格化服饰（晚礼服、常服科技风、国风旗袍、海滩比基尼、经典女仆、清凉泳装、职场 OL、梦幻婚纱、裹尸布装）以增量补丁 `.vrmaddon` 分发（单套仅 1~4 MB，婚纱含复杂大裙摆骨骼 13 MB）。
...
│   │   ├── xiaochun_dinner_dress.vrmaddon  # 晚礼服
│   │   ├── xiaochun_office_lady.vrmaddon   # 职场 OL
│   │   ├── xiaochun_techwear.vrmaddon      # 常服科技风
│   │   ├── xiaochun_wedding.vrmaddon       # 梦幻婚纱 (13 MB)
│   │   └── xiaochun_shroud.vrmaddon       # 裹尸布装 (自带 per-outfit bodyMorph baseline)
